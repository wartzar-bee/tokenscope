...
|---|----------|-----------------|-------------|--------|
| 1 | **punkpeye/awesome-mcp-servers** (the dominant awesome list, >50k★) | Fork → add one alphabetical line under the right category in `README.md` → PR | ✅ PR via PAT | artifact prepared (see `listings/awesome-mcp-servers.entry.md`) |
| 2 | **wong2/awesome-mcp-servers** (2nd major awesome list) | Fork → add entry to `README.md` → PR | ✅ PR via PAT | artifact prepared (same entry, that list's format) |
...
## Autonomy summary
- **Autonomous now (our GitHub PAT):** PRs to the awesome lists (#1, #2). These are the highest-value, lowest-friction, on-policy listings.
- **Passive (no action / crawler-driven):** Glama, mcp.so, PulseMCP mirrors — driven by a public repo with the `mcp` topic + a published npm package.
